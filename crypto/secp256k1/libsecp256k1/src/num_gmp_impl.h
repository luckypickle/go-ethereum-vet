...
#ifdef VERIFY
static void vet_secp256k1_num_sanity(const vet_secp256k1_num *a) {
    VERIFY_CHECK(a->limbs == 1 || (a->limbs > 1 && a->data[a->limbs-1] != 0));
}
#else
#define vet_secp256k1_num_sanity(a) do { } while(0)
#endif
...
static void vet_secp256k1_num_add_abs(vet_secp256k1_num *r, const vet_secp256k1_num *a, const vet_secp256k1_num *b) {
...
static void vet_secp256k1_num_sub_abs(vet_secp256k1_num *r, const vet_secp256k1_num *a, const vet_secp256k1_num *b) {
...
static void vet_secp256k1_num_mod(vet_secp256k1_num *r, const vet_secp256k1_num *m) {
    vet_secp256k1_num_sanity(r);
    vet_secp256k1_num_sanity(m);
    ...
    if (r->neg && (r->limbs > 1 || r->data[0] != 0)) {
        vet_secp256k1_num_sub_abs(r, m, r);
        r->neg = 0;
    }
}
...
static int vet_secp256k1_num_eq(const vet_secp256k1_num *a, const vet_secp256k1_num *b) {
    ...
    if ((a->neg && !vet_secp256k1_num_is_zero(a)) != (b->neg && !vet_secp256k1_num_is_zero(b))) {
        return 0;
    }
    return mpn_cmp(a->data, b->data, a->limbs) == 0;
}

static void vet_secp256k1_num_subadd(vet_secp256k1_num *r, const vet_secp256k1_num *a, const vet_secp256k1_num *b, int bneg) {
    if (!(b->neg ^ bneg ^ a->neg)) { /* a and b have the same sign */
        r->neg = a->neg;
        if (a->limbs >= b->limbs) {
            vet_secp256k1_num_add_abs(r, a, b);
        } else {
            vet_secp256k1_num_add_abs(r, b, a);
        }
    } else {
        if (vet_secp256k1_num_cmp(a, b) > 0) {
            r->neg = a->neg;
            vet_secp256k1_num_sub_abs(r, a, b);
        } else {
            r->neg = b->neg ^ bneg;
            vet_secp256k1_num_sub_abs(r, b, a);
        }
    }
}
